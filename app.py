...
# Function to load and encode the image with fallback
...
possible_paths = [
    "download (1).jpeg",  # Current directory
    os.path.join("Streamlit_Project", "download (1).jpeg"),  # Streamlit_Project directory
    os.path.join("D:", "Quarter 3", "password strenght", "download (1).jpeg"),  # Absolute path
    os.path.join("D:", "Quarter 3", "password strenght", "Streamlit_Project", "download (1).jpeg"),  # Another absolute path
]
...
# Try to get the background image without displaying any messages
background_image = None
if image_path:
    try:
        background_image = get_base64_encoded_image(image_path)
    except Exception:
        # Silently fail and use the fallback background
        pass

# Determine background style based on whether we found an image
if background_image:
    background_style = f"""
...
# Custom CSS styling with dynamic background
st.markdown(f"""
    <style>
    /* Base styling */
    * {{
